...
Arguments:
	...
	stat_size : size of stat embedding required for the CBN layer
	emb_size : size of hidden layer of MLP used to predict beta and gamma values
Returns:
	model : requried resnet model
'''
def create_resnet(n, stat_size, emb_size, use_pretrained):
	if n == 18:
		model = resnet18(stat_size, emb_size, pretrained=use_pretrained).cuda()
	if n == 34:
		model = resnet34(stat_size, emb_size, pretrained=use_pretrained).cuda()
	if n == 50:
		model = resnet50(stat_size, emb_size, pretrained=use_pretrained).cuda()
	if n == 101:
		model = resnet101(stat_size, emb_size, pretrained=use_pretrained).cuda()
	if n == 152:
		model = resnet152(stat_size, emb_size, pretrained=use_pretrained).cuda()
...
class Net(nn.Module):
	...
	def __init__(self, config, no_words, no_answers, resnet_model, stat_size, emb_size, use_pretrained=True):
		...
		self.stat_size = stat_size # stat emb size to be passed to CBN layer
	...
	'''
	def forward(self, image, bn_stats, glove_emb, labels=None):

		####### Question Embedding #######
		# get the lstm representation of the final state at time t
		tgt_mean = bn_stats["mean"]
		tgt_variance = bn_stats["variance"]
